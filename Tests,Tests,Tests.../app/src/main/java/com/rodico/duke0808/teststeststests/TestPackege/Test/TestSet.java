...
public class TestSet {
    ...
    static public ArrayList<Test> getTests(){
        ...
        tests.add(getTestJealous());
        tests.add(getTestKnight());
    ...
    public static Test getTestJealous(){
        ArrayList<TestQuestion> questions = new ArrayList<>();

        String[] answers1;
        answers1 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Если ваш партнер бросает взгляд на незнакомую женщину, вы устраиваете ему скандал прямо на улице?"
                ,2,answers1,0));
        String[] answers2;
        answers2 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Если ваш партнер опаздывает на ужин, вы уверены, что он был с другой?"
                ,2,answers2,0));
        String[] answers3;
        answers3 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Вы расспрашиваете его о работе, о коллегах?"
                ,2,answers3,0));
        String[] answers4;
        answers4 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Вы считаете, что каждую свободную минуту должны проводить вместе?"
                ,2,answers4,0));
        String[] answers5;
        answers5 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Он для вас — свет в окошке?"
                ,2,answers5,0));
        String[] answers6;
        answers6 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Случается ли вам проверять его корреспонденцию и рыться в его вещах?"
                ,2,answers6,0));
        String[] answers7;
        answers7 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Чем чаще он говорит о своих чувствах, тем меньше вы верите?"
                ,2,answers7,0));
        String[] answers8;
        answers8 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Вы хотите, чтобы он интересовался только тем, чем интересуетесь вы?"
                ,2,answers8,0));
        String[] answers9;
        answers9 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Вы всегда спрашиваете у него, куда он ходит и с кем встречается?"
                ,2,answers9,0));
        String[] answers10;
        answers10 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Если вы на него обижены, то молчите по нескольку дней?"
                ,2,answers10,0));
        String[] answers11;
        answers11 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Вас мучают мысли о его бывшей возлюбленной?"
                ,2,answers11,0));
        String[] answers12;
        answers12 = new String[]{ "Да", "Нет"};
        questions.add(new TestQuestion("Он утверждает, что не ревнует вас, потому что доверяет. Для вас это означает, что любовь прошла?"
                ,2,answers12,0));

        Test test  = new Test("Ревнивы ли Вы? (ТЕСТ ДЛЯ ЖЕНЩИН)","Часто ли у вас возникает желание перелистать его записную книжку или проверить содержимое его карманов? Если вы опасаетесь, что в будущем ваша нерациональная ревность возьмет верх над рассудком, и искренне хотите этого избежать, то этот тест — для вас. Отвечайте только «да» или «нет».", questions){
            @Override
            public String getResult() {
                if (this.getCounterOfRight()>=10){
                    return "Вы болезненно ревнивы. Не думайте, что если избранник вас любит, то он автоматически становится вашей собственностью. Вы считаете себя непривлекательной и боитесь, что он бросит вас ради какой нибудь красавицы. Вы ни в чем не уверены, особенно в нем. Задумайтесь над этим, потому что нельзя быть настолько ревнивой и агрессивной, это может привести к конфликтам и даже к разрыву отношений.";
                }
                if (this.getCounterOfRight()>=5&&this.getCounterOfRight()<10){
                    return "Ваша ревность действует на вас мобилизующе, но не она одна управляет вашим поведением. В минуту слабости случается и вам устраивать скандалы. Но, успокоившись, вы понимаете, что для вашей ревности не было никаких оснований.";
                }
                if (this.getCounterOfRight()>=0&&this.getCounterOfRight()<=4){
                    return "Вам совершенно незнакомо чувство ревности. Но тревога и беспокойство могут накапливаться со временем. Вы должны решать волнующие вас проблемы со своим партнером.";
                }
    ...
    public static Test getTestKnight() {
        ArrayList<TestQuestion> questions = new ArrayList<>();

        String[] answers1;
        answers1 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Когда люди моего интеллектуального уровня достигают больших успехов, чем я, меня это очень раздражает."
                , 3, answers1, 0));
        String[] answers2;
        answers2 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion(" Для меня важна высокая оценка моих достижений другими людьми."
                , 3, answers2, 0));
        String[] answers3;
        answers3 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я часто смотрю познавательные телепередачи, даже если это не доставляет мне удовольствия."
                , 3, answers3, 0));
        String[] answers4;
        answers4 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я стараюсь развить некоторые свои качества постоянными тренировками."
                , 3, answers4, 0));
        String[] answers5;
        answers5 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Мне нравится решать сложные задачи."
                , 3, answers5, 0));
        String[] answers6;
        answers6 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я всегда ставлю перед собой конкретные цели и стремлюсь к их достижению."
                , 3, answers6, 0));
        String[] answers7;
        answers7 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я часто трачу на работу свое личное время."
                , 3, answers7, 0));
        String[] answers8;
        answers8 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Мне хотелось бы больше знать о том, как можно развить свои способности."
                , 3, answers8, 0));
        String[] answers9;
        answers9 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Когда я учился в школе, меня часто не удовлетворяли полученные результаты."
                , 3, answers9, 0));
        String[] answers10;
        answers10 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("У меня никогда не получалось проигрывать «красиво»."
                , 3, answers10, 0));
        String[] answers11;
        answers11 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Мне доставляет удовольствие сравнивать черты своего характера с чертами характера других людей."
                , 3, answers11, 0));
        String[] answers12;
        answers12 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Меня скорее всего не привлечет деятельность, не предполагающая широкой известности."
                , 3, answers12, 0));
        String[] answers13;
        answers13 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я очень чувствителен к критике."
                , 3, answers13, 0));
        String[] answers14;
        answers14 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я недоволен имеющимися у меня знаниями и умениями."
                , 3, answers14, 0));
        String[] answers15;
        answers15 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Мне хотелось бы быть лучшим в классе (группе, коллективе)."
                , 3, answers15, 0));
        String[] answers16;
        answers16 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Люди, достигшие больших успехов, для меня более ценны."
                , 3, answers16, 0));
        String[] answers17;
        answers17 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Мне нравятся игры в компании, я всегда пытаюсь в них выиграть."
                , 3, answers17, 0));
        String[] answers18;
        answers18 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Профессиональная карьера является для меня одной из главных ценностей."
                , 3, answers18, 0));
        String[] answers19;
        answers19 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я предпочел бы менее стабильную работу, но с большей вероятностью карьерного роста."
                , 3, answers19, 0));
        String[] answers20;
        answers20 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я хотел бы занимать руководящую должность."
                , 3, answers20, 0));
        String[] answers21;
        answers21 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Человек, достигший больших успехов в жизни, изначально мне симпатичен."
                ,3,answers21,0));
        String[] answers22;
        answers22 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я хочу добиться большего, чем мои родители."
                ,3,answers22,0));
        String[] answers23;
        answers23 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Если бы я был спортсменом, я бы не участвовал в соревнованиях, в которых обречен на поражение."
                ,3,answers23,0));
        String[] answers24;
        answers24 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Если бы я был начальником, я бы пытался быть лучше своих сотрудников во всех областях."
                ,3,answers24,0));
        String[] answers25;
        answers25 = new String[]{"Да","Отчасти", "Нет"};
        questions.add(new TestQuestion("Я удовлетворен собой, только если совершил что то значительное."
                ,3,answers25,0));
        String[] answers26;
        answers26 = new String[]{"менее 16 лет","17–21 год", "22–30 лет", "Более 30 лет"};
        questions.add(new TestQuestion("Укажите Ваш возраст, пожалуйста:"
                ,4,answers26,0));

        Test test = new Test("Насколько вы честолюбивы?", "Попытайтесь максимально честно ответить на следующие 25 утверждений. Если какая то ситуация не слишком близка вам, попробуйте представить, как бы вы вели себя в подобных обстоятельствах.", questions){
            @Override
            public String getResult() {
                int counter=0;
                for (int i=0;i<25;i++){
                    switch (this.getQuestionById(i).answeredId){
                        case 0: counter+=2; break;
                        case 1: counter++; break;
                        case 2:  break;
                    }
                }

                switch (this.getQuestionById(25).answeredId){
                    case 0:
                        if (counter>=40&&counter<=50){
                            return "Честолюбие выражено очень сильно. Ваше жизненное кредо — все или ничего. Вы стремитесь к успеху любой ценой и болезненно реагируете на успехи других людей. Малейшая неудача негативно отражается на вашей самооценке. А это уже попахивает навязчивой идеей. Пора немного ослабить напряжение. Постоянная нервозность отнюдь не способствует достижению блестящих результатов, и вместо долгожданного признания можно получить нервное истощение.";
                        }
                        if (counter>=36&&counter<=39){
                            return "Честолюбие выражено сильно. Вам хочется, чтобы люди хвалили вас за ваши успехи. Это, конечно, неплохо, но неужели вы думаете, что вас больше не за что любить? Тогда вы себя явно недооцениваете.";
                        }
                        if (counter>=23&&counter<=35){
                            return "Честолюбие выражено средне с тенденцией к «сильно». Вы знаете себе цену и стараетесь в выгодном свете подать себя окружающим. Есть много вершин, о покорении которых вы периодически задумываетесь. Ваше честолюбие в основном идет вам на пользу, стимулируя к достижению новых результатов и улучшению старых.";
                        }
                        if (counter>=19&&counter<=22){
                            return "Честолюбие выражено средне с тенденцией к «слабо». Вам хорошо известно, на что вы способны и на что можете рассчитывать. Вы, как говорится, звезд с неба не хватаете. Что ж, если вы удовлетворены, если это то, чего вам действительно хочется от жизни, вам можно только позавидовать.";
                        }
                        if (counter>=0&&counter<=18){
                            return "Честолюбие выражено слабо. Достижение успеха не имеет для вас большого значения. Вы не жаждете добиться любви окружающих, демонстрируя им блестящие результаты. И это скорее всего сказывается на вашей профессиональной деятельности не лучшим образом. Встряхнитесь, проявите интерес к жизни, покажите всем, на что вы способны!";
                        }
                        break;
                    case 1:
                        ...
                        if (counter>=31&&counter<=34){
                            return "Честолюбие выражено сильно. Вам хочется, чтобы люди хвалили вас за ваши успехи. Это, конечно, неплохо, но неужели вы думаете, что вас больше не за что любить? Тогда вы себя явно недооцениваете.";
                        }
                        if (counter>=22&&counter<=30){
                            return "Честолюбие выражено средне с тенденцией к «сильно». Вы знаете себе цену и стараетесь в выгодном свете подать себя окружающим. Есть много вершин, о покорении которых вы периодически задумываетесь. Ваше честолюбие в основном идет вам на пользу, стимулируя к достижению новых результатов и улучшению старых.";
                        }
                    ...
                    case 3:
                        ...
                        if (counter>=35&&counter<=39){
                            return "Честолюбие выражено средне с тенденцией к «сильно». Вы знаете себе цену и стараетесь в выгодном свете подать себя окружающим. Есть много вершин, о покорении которых вы периодически задумываетесь. Ваше честолюбие в основном идет вам на пользу, стимулируя к достижению новых результатов и улучшению старых.";
                        }
                        if (counter>=23&&counter<=27){
                            return "Честолюбие выражено средне с тенденцией к «слабо». Вам хорошо известно, на что вы способны и на что можете рассчитывать. Вы, как говорится, звезд с неба не хватаете. Что ж, если вы удовлетворены, если это то, чего вам действительно хочется от жизни, вам можно только позавидовать.";
                        }
